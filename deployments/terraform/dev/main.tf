...
# terraform apply -var="artifact_bucket=artifact-bucket-gin-test" -var="artifact_zip_name=deployment" -auto-approve
# terraform apply
...
resource "aws_lambda_function" "weather_api" {
  ...
  s3_key    = "${var.artifact_binary_name}.zip"

  handler = "${var.artifact_binary_name}"
  runtime = "go1.x"
